...
이 폴더에서 `pnpm check`, `pnpm verify`, `pnpm dev`, `pnpm build`, `pnpm start`, `pnpm lint`, `pnpm lint:fix`, `pnpm typecheck`, `pnpm format:check`, `pnpm format`을 실행합니다. start는 빌드 후 실행합니다.

`GET /health`는 constructor DI로 주입된 서비스에서 `{ "status": "ok" }`를 반환합니다. DB readiness는 검사하지 않습니다. `PORT`는 기본 8080이며 프로세스 환경으로 전달합니다. 예: `PORT=4001 pnpm start`. `.env.example`은 자동 로딩하지 않습니다.
...
named import 정렬은 Oxlint 내장 규칙, import 선언·그룹 정렬과 포맷·package.json 정렬은 Oxfmt가 담당합니다. ESLint와 외부 lint 플러그인은 사용하지 않습니다. `check`는 소스를 수정하지 않고 검사하며, `verify`는 check 후 이 앱을 빌드합니다. 자동수정은 `pnpm lint:fix && pnpm format` 후 `pnpm check`로 확인합니다.

[전체 구성과 검사 한계](../../README.md)를 참고하세요.

## Swagger와 Zod

Swagger UI는 <http://localhost:8080/docs>, OpenAPI JSON은 <http://localhost:8080/docs-json>입니다. API 프로세스가 문서를 제공하며 현재 템플릿에서는 실행 환경과 무관하게 활성화됩니다.
